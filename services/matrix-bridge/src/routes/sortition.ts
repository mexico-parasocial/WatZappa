...
export async function apiSortitionRunsPOSTHandler(
  ...
  // F9: creating a run is a governance action.
  if (
    !(await authorizeOrRespond(ctx, res, auth.did, 'sortition.run', {
      kind: 'community',
      communityUri,
    }))
  )
  ...
  if (![50, 100, 500].includes(size)) {
    res.writeHead(400, { 'Content-Type': 'application/json' })
    res.end(JSON.stringify({ error: 'Assembly size must be 50, 100, or 500' }))
    return
  }
...
export async function apiSortitionRunsGETHandler(
  req: IncomingMessage,
  res: ServerResponse,
  ctx: RouteContext,
): Promise<void> {
  ...
  // F9: the selected-members listing is member-only (the proofs
  // themselves stay publicly auditable by decision).
  if (
...
export async function apiSortitionRunsProcessHandler(
  req: IncomingMessage,
  res: ServerResponse,
  ctx: RouteContext,
): Promise<void> {
...
export async function apiSortitionProofsHandler(
  req: IncomingMessage,
  res: ServerResponse,
  ctx: RouteContext,
): Promise<void> {
...
export async function apiSortitionProofHandler(
  req: IncomingMessage,
  res: ServerResponse,
  ctx: RouteContext,
): Promise<void> {
...
export async function apiVerifySortitionHandler(
  req: IncomingMessage,
  res: ServerResponse,
  ctx: RouteContext,
): Promise<void> {
...
export async function apiSortitionProofAsRecordHandler(
  req: IncomingMessage,
  res: ServerResponse,
  ctx: RouteContext,
): Promise<void> {
