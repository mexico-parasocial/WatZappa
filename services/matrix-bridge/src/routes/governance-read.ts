...
export async function apiConstitutionHandler(
  req: IncomingMessage,
  res: ServerResponse,
  ctx: RouteContext,
): Promise<void> {
  ...
  try {
    await authorize(ctx, auth.did, 'community.read', {
      kind: 'community',
      communityUri,
    })
  } catch (err) {
...
export async function apiProposalsHandler(
  req: IncomingMessage,
  res: ServerResponse,
  ctx: RouteContext,
): Promise<void> {
...
export async function apiDecisionsHandler(
  req: IncomingMessage,
  res: ServerResponse,
  ctx: RouteContext,
): Promise<void> {
