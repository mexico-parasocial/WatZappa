...
import { authorizeOrRespond, authorizeUserReadOrRespond } from '../authz.js'
...
/** POST /api/cards */
export async function apiCardsPOSTHandler(
  req: IncomingMessage,
  res: ServerResponse,
  ctx: RouteContext,
): Promise<void> {
...
export async function apiCardsGETHandler(
  req: IncomingMessage,
  res: ServerResponse,
  ctx: RouteContext,
): Promise<void> {
...
export async function apiCommunityTreeContributionsPOSTHandler(
  req: IncomingMessage,
  res: ServerResponse,
  ctx: RouteContext,
): Promise<void> {
...
export async function apiCommunityTreeContributionsGETHandler(
  req: IncomingMessage,
  res: ServerResponse,
  ctx: RouteContext,
): Promise<void> {
  ...
  if (
    ...
    return
  if (viewerDid) {
    await authorizeUserReadOrRespond(
      ctx,
      res,
      auth.did,
      viewerDid,
...
export async function apiCommunityTreeContributionsVoteHandler(
  req: IncomingMessage,
  res: ServerResponse,
  ctx: RouteContext,
): Promise<void> {
  ...
  if (!contributionId || !voterDid || !['approve', 'reject'].includes(vote)) {
    res.writeHead(400, { 'Content-Type': 'application/json' })
    res.end(
      JSON.stringify({
  ...
  const existing = await ctx.db.getCommunityMapContribution(contributionId)
  if (!existing) {
    res.writeHead(404, { 'Content-Type': 'application/json' })
    res.end(JSON.stringify({ error: 'Contribution not found' }))
    return
  }
  // F9: voting requires active membership in the contribution's community.
  if (
    !(await authorizeOrRespond(ctx, res, auth.did, 'community.contribute', {
      kind: 'community',
      communityUri: existing.communityUri,
    }))
  )
    return
...
export async function apiRelationshipsHandler(
  req: IncomingMessage,
  res: ServerResponse,
  ctx: RouteContext,
): Promise<void> {
...
export async function apiGraphHandler(
  req: IncomingMessage,
  res: ServerResponse,
  ctx: RouteContext,
): Promise<void> {
...
export async function apiSuggestionsHandler(
  req: IncomingMessage,
  res: ServerResponse,
  ctx: RouteContext,
): Promise<void> {
...
export async function apiSuggestionsAcceptHandler(
  req: IncomingMessage,
  res: ServerResponse,
  ctx: RouteContext,
): Promise<void> {
  ...
  const suggestion = await ctx.db.getSuggestion(id)
  if (!suggestion) {
    res.writeHead(404, { 'Content-Type': 'application/json' })
    res.end(JSON.stringify({ error: 'Suggestion not found' }))
    return
  }
  const sourceCard = await ctx.db.getCard(suggestion.sourceCardId)
  if (!sourceCard) {
    res.writeHead(404, { 'Content-Type': 'application/json' })
    res.end(JSON.stringify({ error: 'Source card not found' }))
    return
  }
  if (
    !(await authorizeOrRespond(ctx, res, auth.did, 'community.contribute', {
      kind: 'community',
      communityUri: sourceCard.communityUri,
    }))
  )
    return
...
export async function apiSuggestionsRejectHandler(
  req: IncomingMessage,
  res: ServerResponse,
  ctx: RouteContext,
): Promise<void> {
...
export async function apiSummarizeHandler(
  req: IncomingMessage,
  res: ServerResponse,
  ctx: RouteContext,
): Promise<void> {
...
/** POST /api/vote */
export async function apiVoteHandler(
  req: IncomingMessage,
  res: ServerResponse,
  ctx: RouteContext,
): Promise<void> {
  ...
  if (!cardId || !voterDid || typeof influence !== 'number') {
    res.writeHead(400, { 'Content-Type': 'application/json' })
    res.end(JSON.stringify({ error: 'Missing cardId, voterDid, or influence' }))
    return
  }
  if (auth.did !== voterDid) {
    res.writeHead(403, { 'Content-Type': 'application/json' })
  ...
  const card = await ctx.db.getCard(cardId)
  if (!card) {
    res.writeHead(404, { 'Content-Type': 'application/json' })
    res.end(JSON.stringify({ error: 'Card not found' }))
    return
  }
  // F9: voting requires active membership in the card's community.
  if (
    !(await authorizeOrRespond(ctx, res, auth.did, 'community.contribute', {
      kind: 'community',
      communityUri: card.communityUri,
    }))
  )
    return
  if (influence < -3 || influence > 3) {
    res.writeHead(400, { 'Content-Type': 'application/json' })
    res.end(JSON.stringify({ error: 'Influence must be between -3 and +3' }))
    return
  }
...
export async function apiVotesHandler(
  req: IncomingMessage,
  res: ServerResponse,
  ctx: RouteContext,
): Promise<void> {
  ...
  if (voterDid) {
    ...
    res.writeHead(200, { 'Content-Type': 'application/json' })
    res.end(JSON.stringify({ votes, totalInfluence, voteCount: votes.length }))
  }
}

/** GET /api/community-pulse (prefix) */
export async function apiCommunityPulseHandler(
  req: IncomingMessage,
  res: ServerResponse,
  ctx: RouteContext,
): Promise<void> {
  ...
  if (
    ...
    return
  if (voterDid) {
    await authorizeUserReadOrRespond(ctx, res, auth.did, voterDid, communityUri)
  }
...
/** POST /api/extract */
export async function apiExtractHandler(
  req: IncomingMessage,
  res: ServerResponse,
  ctx: RouteContext,
): Promise<void> {
  ...
  if (auth.did !== authorDid) {
    res.writeHead(403, { 'Content-Type': 'application/json' })
    res.end(JSON.stringify({ error: 'authorDid must match authenticated DID' }))
    return
  }
  if (
    !(await authorizeOrRespond(ctx, res, auth.did, 'community.contribute', {
