...
for (const buildServer of [buildMethodLexicons, buildAddLexicons]) {
  describe(buildServer, () => {
    ...
    it('handles deeply nested output', async () => {
      const deepServer = await buildServer(LEXICONS, handlers, {
        validateResponse: false,
        payload: {
          blobLimit: BLOB_LIMIT,
        },
      })
      const deepHttpServer = await createServer(deepServer)
      const { port } = deepHttpServer.address() as AddressInfo
      const deepUrl = `http://localhost:${port}`

      // @NOTE not using the client to avoid any potential issues with JSON
      // processing in the client.
      try {
        const res = await fetch(
          `${deepUrl}/xrpc/io.example.deeplyNestedStructure`,
          {
            method: 'post',
        ...
        await closeServer(deepHttpServer)
      }
    })
  })
}
